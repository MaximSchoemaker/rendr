...
const sketch = rendr.createSketch(sketch => (tick_par) => {
   ...
   function drawCircles(view, count, steps) {
   ...
   function drawLines(view, count, steps) {
   ...
   const frame_par = sketch.draw(() => {
      ...
      if (finalState == null) return view;

      const { steps, vals } = finalState;
      const count = vals.length;

      // drawCircles(view, count, steps);
      drawLines(view, count, steps);
...
export default rendr.createSetup("/sketches/sort.js", createUI => {
   ...
   createUI(ui => {
      ui.createContainer(ui => {
         ui.createWindow(ui => {
            ui.createParameterNumber("count", count_par, { min: 1, max: 100, step: 1 });
         });
