...
export default createSketch<Props>((engine, ui, props) => {
  ...
  if (ANIMATION || VIDEO) {
    ...
    for_n(FRAMES, i => animation(i / FRAMES, initial_state));

    const state = engine.simulate(initial_state, FRAMES, (positions, props) => {
      const { index } = props;
      const t = index / FRAMES;
      animation(t, positions);
    });
...
function animation(t: number, positions: Position[]) {
