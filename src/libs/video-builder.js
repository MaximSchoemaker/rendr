...
function blobToDataURL(blob, callback) {
	return new Promise((resolve, reject) => {
		...
		a.onerror = function (e) { console.error(e); reject(); }
...
(function (global) {
	...
	VideoBuilder.prototype = {
		setup: function (config) {
			...
			this.movieDesc.quality = config.quality;
		},

		addCanvasFrame: async function (canvas) {
			let u;
			if (canvas.convertToBlob) {
				try {
					const preBlob = await canvas.convertToBlob({ type: 'image/jpeg', quality: this.movieDesc.quality });
					u = await blobToDataURL(preBlob);
				} catch {
					// console.log("did not add canvas", preBlob);
					// return;
				}
			}

			if (u == null)
				u = canvas.toDataURL('image/jpeg', this.movieDesc.quality);
		...
		build: async function (avi, onFinish) {
			...
			if (typeof window !== "undefined") {
				...
				// var buf = Buffer.alloc(blob, 'base64');
