...
export function createCache<T>(count = 0): Cache<T> {
   ...
   function mutate(action: Action) {
      ...
      if (index !== undefined) {
         ...
         if (key === "value") cache[index].last_set_timestamp = Date.now()
         ...
         if (this[_key] === value) return// @ts-ignore
         this[_key] = value;

         if (key === "value") last_set_timestamp = Date.now()
...
type SketchReturn = { [key: string]: Dependency };
type SketchInitArgs = Dependency[]
type SketchFn = (sketch: Sketch) => SketchReturn | ((...args: SketchInitArgs) => SketchReturn);

export function createSketch(fn: SketchFn) {

   return {
      init(...args: SketchInitArgs) {
...
type Dependencies_ids_and_indexes = { id: number, index: number }[]
